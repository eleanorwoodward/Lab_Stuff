...
# 
# 
# recurrent.neo <- clean.neoantigen[clean.neoantigen$pair_id %in% c(unique(recurrent.ccfs$pair_id),"MEN0122-RBM-P1", "MEN0122-RBM-P2", "MEN0122-RBM-P3",
#                                                                   "MEN0122-RBM-P4", "MEN0122-RBM-P5"), ]
# rownames(recurrent.neo) <- 1:nrow(recurrent.neo)
# colnames(men.mafs[[i]])[5] <- "MEN0101.TumorB"
# ## look at interplay between changes between recurrences and neo-antigen load
# recurrent.neo$one.hit <- 0
# for (i in 2:length(men.mafs)){
#     maf <- men.mafs[[i]]
#     
#     ## change colnames back
#     for (j in 5:ncol(maf)){
#         pair.name <- master.table[master.table$Tumor.Name == colnames(maf)[j], ]$Pair.Name[1]
#         colnames(maf)[j] <- pair.name
#     }
#     
#     maf.vals <- maf[, -c(1:4)]
#     maf.vals <- apply(maf.vals,2,as.numeric)
#     goners <- rowSums(maf.vals) == 1
#     one.hit <- maf[goners, ]
#     
#     for (k in 1:nrow(one.hit)){
#         sample <- colnames(one.hit)[4 + which(one.hit[k, -(1:4)] == 1)]
#         gene <- one.hit[k, 1]
#         pos <- one.hit[k, 4]
#         
#         hit <- recurrent.neo[recurrent.neo$pair_id == sample & recurrent.neo$Hugo_Symbol == gene & recurrent.neo$Start_position == pos, ]
#         if (nrow(hit) == 0){
#             #nothing
#         }else if (nrow(hit) == 1){
#             recurrent.neo[rownames(hit)[1], 9] <- 1
#         }else{
#             recurrent.neo[rownames(hit), 9] <- 1
#             print(c(i, sample))
#         }
#     }
#     
# }
# 
# View(recurrent.neo[duplicated(recurrent.neo$Start_position), ])
# 
# recurrent.neo <-  recurrent.neo[recurrent.neo$pair_id %in% unique(recurrent.neo$pair_id)[c(1:16, 19:24)], ]
# 
# fisher.test(table(recurrent.neo$one.hit, recurrent.neo$ic50 < 500))
# 
# 



## nycflights walkthrough
dim(flights)
head(flights)
filter(flights, month == 12, dep_time < 100)
slice(flights, 2:4)
arrange(flights, year, carrier)
