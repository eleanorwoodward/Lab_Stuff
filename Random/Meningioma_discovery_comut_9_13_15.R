...
# Load SIF
master_sif = read.delim('/xchip/beroukhimlab/agarwalla/meningioma/analysis/sif/meningioma_discovery_sif_20150915.txt', stringsAsFactors = FALSE)

#Prepare co-mut rows and type of alteration
sig_gene_list=c('grade', 'NF2', 'AKT1', 'CDKN2C', 'KLF4', 'TRAF7')
alt_type=c('grade', 'mut', 'mut', 'mut', 'mut', 'mut')
...
assign_plot_col<-function(maf,mut_matches){
	...
	missense_col=rgb(61, 117, 164,maxColorValue=255) # Blue
...
assign.location.color <- function(location){
	# Set the plotting defaults for color
	null_col='gainsboro'
	vestibular.color = rgb(131, 76, 139, maxColorValue=255) # Purple
	peripheral.color = rgb(205, 39, 48, maxColorValue=255) # Red
	trigeminal.color = 'dodgerblue4'
	jugular.color = rgb(244, 232, 83,maxColorValue=255) # yellow

	# rgb(228, 118, 54, maxColorValue=255) # Light Orange
	# rgb(61, 117, 164,maxColorValue=255) # Blue
	# rgb(244, 232, 83,maxColorValue=255) # yellow
	
	if (location == 'vestibular'){plot_col=vestibular.color}
	if (location == 'peripheral'){plot_col=peripheral.color}
	if (location == 'trigeminal'){plot_col=trigeminal.color}
	if (location == 'jugular'){plot_col=jugular.color}

return(plot_col)
}

assign.grade.color <- function(grade){
	null_col='gainsboro'
	grade1.color = rgb(131, 76, 139, maxColorValue=255) # Purple
	grade2.color = rgb(205, 39, 48, maxColorValue=255) # Red
	grade3.color = 'dodgerblue4'

	# rgb(228, 118, 54, maxColorValue=255) # Light Orange
	# rgb(61, 117, 164,maxColorValue=255) # Blue
	# rgb(244, 232, 83,maxColorValue=255) # yellow
	
	if (grade == 0){plot_col=null_col}
	if (grade == 1){plot_col=grade1.color}
	if (grade == 2){plot_col=grade2.color}
	if (grade == 3){plot_col=grade3.color}
...
#NF2_begin=29999545
#NF2_end = 30094589
#loss_list = master_sif$loss
#cnLOH_list = master_sif$cnLOH
#loh_list = master_sif$LOH
#n_mut = master_sif$rate_per_MB
#location_list = master_sif$location
#location.order = master_sif$location.order
#NF2_mut_list=c()
grade.list = master_sif$grade
master.maf = read.delim('/xchip/beroukhimlab/agarwalla/meningioma/analysis/comut/meningioma.all.muts.20150915.maf', stringsAsFactors = FALSE, comment.char = "#")
...
# 	individual.index <- grep(master_sif$individual_id[i], master.maf$Tumor_Sample_Barcode)
...
#new_order=rev(order(location.order, NF2_mut_list, loh_list, loss_list, cnLOH_list, n_mut))
new_order = order(master_sif$case_sample)
reordered_master_sif=master_sif[new_order,]
unique_individuals=reordered_master_sif$case_sample
box_width=0.9


output_fn='/xchip/beroukhimlab/agarwalla/meningioma/analysis/comut/meningioma_co_mut_9_15_15v2.pdf'
...
axis(side=1,at=x_label_positions,labels=x_labels,las=2,cex.axis=0.5)
...
for(i in 1:nrow(reordered_master_sif)){
	...
	for (j in 1:length(sig_gene_list)){
			...
			if(alt_type[j]=='location'){
				if (g=='location'){
					plot_col = assign.location.color(location_list[new_order[i]])
				}
				rect(xleft=i-box_width/2,xright=i+box_width/2,ybottom=j-box_width/2,ytop=j+box_width/2,col=plot_col,border=NA)					
			}
			...
			if(alt_type[j]=='grade'){
				if (g=='grade'){
					plot_col = assign.grade.color(grade.list[new_order[i]])
				}
				rect(xleft=i-box_width/2,xright=i+box_width/2,ybottom=j-box_width/2,ytop=j+box_width/2,col=plot_col,border=NA)					
			}
	...
	#n_mutations=reordered_master_sif[i,'rate_per_MB']
